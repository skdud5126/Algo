...
가능할 경우 1. [가장 먼 노드], 불가능할 경우 0을 출력한다.
'''
import sys
...
for _ in range(1,11):  # 10개의 테스트 케이스
    case, road_count = map(int,input().split())  # 테스트 케이스 & 길의 총 개수
    arr = list(map(int,input().split()))   # 길

    avjL = [[] for _ in range(100)]

    for i in range(road_count):
        v1, v2 = arr[i*2], arr[i*2+1]
        avjL[v1].append(v2)
    print(f'#{case} {DFS(0,len(avjL))}')
