...
for i in range(1, T + 1):
    ...
    for j in range(2, N - 2):
        if (arr[j - 2] < arr[j]) and (arr[j - 1] < arr[j]) and (arr[j + 1] < arr[j]) and (arr[j + 2] < arr[j]):
            num_list = [arr[j - 2], arr[j - 1], arr[j + 1], arr[j + 2]]
            max_floor = num_list[0]

            for x in num_list:
                if max_floor < x:
                    max_floor = x
            total += arr[j] - max_floor
